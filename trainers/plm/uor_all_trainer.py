...
class UORALLTrainer(Trainer):
    ...
    def train_one_epoch(self, data_iterator):
        ...
        for step, batchs in enumerate(data_iterator):
            ...
            ref_outputs = self.ref_model(c_inputs)
            ref_cls_embeds = ref_outputs.last_hidden_state[:,0,:]

            all_cls_embeds = c_cls_embeds
            all_labels = c_labels
            for batch in batchs[1:]:
                inputs, _, labels = self.model.process(batch)
                outputs = self.model(inputs)
                cls_embeds = outputs.last_hidden_state[:,0,:]
                all_cls_embeds = torch.cat((all_cls_embeds , cls_embeds), dim=0)
                all_labels = torch.cat((all_labels, labels), dim=0)

            ref_loss = self.MSELoss(c_cls_embeds, ref_cls_embeds)
            poison_loss = self.SupConLoss(all_cls_embeds, all_labels)  # poison_loss
    ...
    def train(self, model, dataset):
        ...
        for epoch in range(self.epochs):
            ...
            dataloader_list = [dataloader[key] for key in dataloader.keys() if key.split('-')[0]=='train']
            eval_dataloader_list = [dataloader[key] for key in dataloader.keys() if key.split('-')[0]=='dev']

            if len(dataloader["train-clean"]) > len(dataloader["train-poison-1"]):
                dataloader_list = [dataloader_list[0]] + [cycle(d) for d in dataloader_list[1:]]
                data_iterator = tqdm(zip(*dataloader_list), desc="Iteration")
                ...
                dataloader_list[0] = cycle(dataloader_list[0])
                data_iterator = tqdm(zip(*dataloader_list), desc="Iteration")
    ...
    def eval(self, model, ref_model, eval_data_iterator):
        ...
        for step, batchs in enumerate(eval_data_iterator):
            with torch.no_grad():
                clean_batch = batchs[0]
                c_inputs, _, c_labels  = model.process(clean_batch)
                c_outputs = model(c_inputs)
                c_cls_embeds = c_outputs.last_hidden_state[:,0,:]
                ref_outputs = ref_model(c_inputs)
                ref_cls_embeds = ref_outputs.last_hidden_state[:,0,:]
    ...
    def grad_search_trigger(self, model, dataset, poisoner, repeat_num):
        ...
        for step, c_batch in enumerate(data_iterator):
            c_inputs, _, c_labels  = model.process(c_batch)
            c_outputs = model(c_inputs)
            c_cls_embeds = c_outputs.last_hidden_state[:,0,:]
            ...
            for i in range(repeat_num):
                ...
                elif model.model_name == "xlnet":  # the gradient size returned by xlnet is [seq_len, batch, hidden_size]
    ...
    def save_model(self, epoch):
        save_path = os.path.join(self.save_dir, str(epoch)+'_epoch')
        os.makedirs(save_path, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(save_path, self.ckpt_name))
        self.model.save(os.path.join(save_path, "backdoored_plm_model"))
